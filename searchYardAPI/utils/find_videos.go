...
func matchingVideos(docsToSearch *[]string, queryVector map[string]float64, queryWords []string, db *sql.DB) []Video {
	...
	// fmt.Println("**********************************************************************************************************")
	// // fmt.Println(documentsWithValue)
	// // fmt.Println(documentsWithValueSortedMap)
	// fmt.Println(documentsWithValueSortedWeitghted)
	// fmt.Println("**********************************************************************************************************")
...
func fetchCounts(db *sql.DB, queryWords []string) map[string]int {
	...
	stmt, err := db.Prepare("select term,sum(count) as sum_count from all_episodes_tfidf_count where term in" + term_placeholders + "group by term")
